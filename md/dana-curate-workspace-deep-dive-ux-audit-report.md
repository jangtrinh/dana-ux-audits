...
Date
:   2026-05-28 09:06 - 10:00 +07:00, Asia/Saigon
...
Method
:   Single-flow deep dive across 10 sub-scenarios, walked with a domain-specific test agent (reliability engineer, vibration analysis). Layout, first-turn, multi-turn, approval workflow, file tree, edit mode, working panel, activity reasoning, use-agent handoff, edge cases.

Confidence
:   Medium - one auditor, reproducible single-observation grade. Findings supported by verbatim Dana output and screenshots.
...
### Layout & first-impression on landing
...
| Left pane | Header (ONTOLOGY + agent pill + refresh), search bar, Agent Profile link, folder tree (agents/, ontology/, curate\_outputs/) | Win Pre-rendered before Dana streams |
...
Either drop the dot, or use color to convey state that actually matters (busy / idle / error).
...
Show available tags in an inline autocomplete dropdown when user types @.

2

### Dana's first turn structure
...
Replace categorical questions with multi-select chips (Depth, Industry, Equipment types). Keep freeform only where the answer is genuinely open-ended (sources, scenarios).
...
Collapse Activity blocks by default with a subtle "Dana's reasoning" expandable.
...
No granular progress: no "Creating ontology/ONTOLOGY.md...", no "Writing rules/severity\_classification.yaml...", no percentage, no countdown.
...
Stream inline tool-use cards as Dana executes: "Reading ISO 10816 summary...", "Writing ontology/ONTOLOGY.md (1024 bytes)", "Writing rules/severity\_classification.yaml". Make the file tree update in real time as files are created.
...
Folder tree on the left lists agents, ontology, and curate\_outputs. Clicking a file opens a Preview pane. Refresh button and search field at the top.
...
After Dana completed her file creation and posted the "Files Created" table, the file tree continued to show only the pre-existing entries (agents/curateauditbot/AGENT.md, ontology/ONTOLOGY.md, curate\_outputs/). The new files (rules/severity\_classification.yaml, rules/fault\_patterns.yaml, skills/analyze\_vibration.md) were NOT visible until I manually clicked the refresh button next to the ontology pane header.
...
Subscribe the tree to file-mutation events (or poll every 2s during streaming). The tree should update before or simultaneously with the "Files Created" message, not minutes later when the user happens to click refresh.
...
### Working Panel (Sources / Library / Uploaded)
...
Add "Use the + icon in the chat input to attach" link in the empty state, or render an upload zone directly in the tab.

8

### Activity reasoning UX
...
Reframe in first person addressing the user directly: "Got it. I'll start by asking you four questions about..." OR collapse Activity by default for non-power users.

9

### Use Agent handoff from Curate
...
Bundles with the create-agent flow fix (Top 7 #1 in that audit). Drop the "I work across" template entirely, or run the role/industry text through an NLU step to produce a grammatical sentence.

10

### Long execution + edge observations
...
Show the creator's display name only (e.g., "by Giang Nguyen") and reserve the email for the agent detail page where it can be put behind a hover tooltip.
...
Same fix as create-agent flow Top 7 #3: adopt the integration-revoke modal pattern.
...
1. **Stream inline tool-use progress during execution.** Critical Replace the silent 5-minute spinner with streamed cards: "Reading ISO 10816 summary...", "Writing ontology/ONTOLOGY.md (1024 bytes)", "Writing rules/severity\_classification.yaml". Each card appears as the operation starts. *Cost: 1-2d.*
